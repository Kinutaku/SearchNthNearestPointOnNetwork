...
# Partially modified by Kinutaku 
# 2023-01-25 
...
def connect_poi(pois, nodes, edges, name, prim_key=0, key_col=None, path=None, threshold=200, knn=5, meter_epsg=3857):
    ...
    node_highway_poi = name
    edge_highway = 'projected_footway'
    osmid_prefix = -len(pois) - prim_key  ### change prefix
    ...
    # nodes_id_dict = dict(zip(nodes_coord, nodes_meter['osmid'].astype('Int64')))  # 数字の若いorg, endを残
    #### 数字の若いorg, endを残
    temp = pd.DataFrame({'nodes_coord':nodes_coord, 'nodes_meter_osmid':nodes_meter['osmid'].astype('Int64')})
    temp = temp.sort_values('nodes_meter_osmid', ascending=False)
    nodes_id_dict = dict(zip(temp.nodes_coord, temp.nodes_meter_osmid))
